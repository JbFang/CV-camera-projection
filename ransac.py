...
def calculate_num_ransac_iterations(prob_success, sample_size, ind_prob_correct):
    ...
    S = math.log(1-prob_success, base)
...
def find_inliers(x_0s, F, x_1s, threshold):
    ...
    for i in range(len(x_0s)):
        err = fundamental_matrix.point_line_distance(np.dot(F,x_1s[i]), x_0s[i])
...
def ransac_fundamental_matrix(x_0s, x_1s):
    ...
    for i in range(num_iteration):
        ...
        x_0_sample, x_1_sample = two_view_data.preprocess_data(x_0s[samples], x_1s[samples])
        ...
        if len(inliers) > best_len or (len(inliers) == best_len and res < best_res):
            best_F = F
